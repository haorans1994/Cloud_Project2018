...
class MyMapView(object):
    ...
    def create_view(self):
        ...
        # map_tweets = "function(doc) {var location = doc.place.full_name.split(','); if(doc.place.place_type == 'neighborhood' && location[1] == ' Canberra') {emit(doc.place.name, [doc.text, doc.user.screen_name, doc.place.place_type, doc.coordinates]);}if(doc.place.place_type == 'city' && location[0] == 'Canberra' && doc.coordinates) {emit(doc.place.name, [doc.text,doc.usentiment, doc.place.place_type, doc.coordinates]);}}"
